...
#define VERSION_MAX_LEN 46
#define VERSION_BBTMANAGER_SERVICE "0.0.1"

namespace android {

int BbtManagerService::getServiceVersion(char *ver, unsigned int len){
    snprintf(ver,len - 1 ,"(%s)-(%s-%s)", VERSION_BBTMANAGER_SERVICE, __DATE__, __TIME__);
    ver[len -1] = '\0';
    return strlen(ver);
}
...
void  BbtManagerService::instantiate() {
	...
	snprintf(ver,VERSION_MAX_LEN - 1 ,"(%s)-(%s-%s)", VERSION_BBTMANAGER_SERVICE, __DATE__, __TIME__);
	ver[VERSION_MAX_LEN -1] = '\0';
    ALOGD("BbtManagerService Instantiate, Version:%s",ver);
}
...
int32_t BbtManagerService::setLedStatus(int32_t eye_up, int32_t eye_down, int32_t mouth, int32_t ear) {
	...
	snprintf(value, VALUE_MAX_LEN,"%d%d%d%d",eye_up, eye_down, mouth, ear);
	ALOGD("setLedStatus: %s",value);
	if((fd = open(SYS_LED_STATUS, O_RDWR)) <= 0) {
		ALOGE("open (%s) failed(%d)\n", SYS_LED_STATUS, fd);
		return -1;
	}
    ret = write(fd, value, VALUE_MAX_LEN);
	if( ret < 0) {
